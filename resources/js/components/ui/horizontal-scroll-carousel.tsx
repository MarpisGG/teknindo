...
interface HorizontalScrollCarouselProps {
  images: { src: string; label: string; link: string }[]; // gunakan link di sini
}
    ...
    <section ref={targetRef} className="relative h-[500vh] w-full">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div style={{ x }} className="flex gap-10">
          {images.map(({ src, label, link }, index) => (
            <Card src={src} label={label} link={link} key={index} />
          ))}
...
const Card: React.FC<{ src: string; label: string; link: string }> = ({ src, label, link }) => {
  return (
    <a href={link} className="group relative w-[80vw] max-w-[720px] aspect-[3/2] overflow-hidden rounded-lg">
      <img src={src} alt={label} className="w-full h-full object-cover opacity-80 transition-transform group-hover:scale-105 duration-500" />
      <div className="absolute inset-0 flex items-center justify-center object-fill md:object-cover bg-opacity-30">
