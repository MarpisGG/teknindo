...
function Products() {
    ...
    // Skeleton Card
    const SkeletonCard = () => (
        <div className="mx-auto my-4 flex w-full max-w-6xl animate-pulse flex-col rounded-2xl bg-gray-200 shadow-lg md:flex-row">
            {/* Left Side */}
            <div className="flex w-full flex-col items-center justify-center p-4 md:w-1/2 md:p-8">
                ...
                <div className="h-48 w-full rounded bg-gray-300 md:h-64"></div>
            </div>
            {/* Right Side */}
            <div className="flex w-full flex-col p-4 md:w-1/2 md:p-8">
                ...
                <div className="mt-auto h-10 w-48 rounded bg-gray-300"></div>
            </div>
        </div>
    );
    ...
    return (
        <>
            ...
            <SliderImageProduct />
            <div className="flex items-center justify-center bg-[#d9d9d9] py-8 text-gray-800">
                    ...
                    {loading
                        ...
                        : types.map((type, idx) => {
                              ...
                              return (
                                  <motion.div
                                      key={type.id}
                                      className={`mx-auto my-4 flex w-full max-w-6xl flex-col rounded-2xl shadow-lg transition-all duration-300 md:flex-row ${isGray ? 'bg-[#fcc200]' : 'bg-white'} hover:scale-[1.02] hover:shadow-xl`}
                                  ...
                                  >
                                      {/* Left Side */}
                                      <div className="flex w-full flex-col items-center justify-center p-4 md:w-1/2 md:p-8">
                                          <h1 className="mb-4 text-center text-2xl font-bold text-gray-900 md:text-3xl">{type.name}</h1>
                                          <div className="flex h-48 w-full items-center justify-center md:h-64">
                                              <img src={`/storage/${type.image}`} alt={type.name} className="max-h-full max-w-full object-contain" />
                                          </div>
                                      </div>
                                      {/* Right Side */}
                                      <div className="flex w-full flex-col p-4 md:w-1/2 md:p-8">
                                          <h3 className="mb-6 text-base text-gray-700 md:text-lg">{type.description}</h3>
                                          <div className="flex">
            ...
            <motion.div className="" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp}>
                <motion.div className="mx-auto bg-[#fcc200]" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp}>
                    <h1 className="mb-4 pt-8 text-center text-3xl font-bold text-gray-900">Discover More of Our Products</h1>
