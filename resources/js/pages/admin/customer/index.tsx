                ...
                axios
                    ...
                    .catch((error) => {
                        if (error.response?.status === 405) {
                            Swal.fire('Warning!', 'The Customer was deleted, but the server returned an unexpected response.', 'warning');
                            window.location.reload();
                        } else {
        ...
        <AppLayout>
            <div className="mx-auto max-w-6xl p-4 sm:p-6">
                <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <h2 className="text-2xl font-bold text-gray-800 md:text-3xl">Customer Management</h2>
                                ...
                                customers.map((customer, idx) => (
                                    <tr key={customer.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        <td className="border-b px-4 py-2">
                                            <img src={`/storage/${customer.image}`} className="h-32 w-auto object-fill" />
                                        </td>
